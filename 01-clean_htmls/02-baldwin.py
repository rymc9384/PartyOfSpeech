# -*- coding: utf-8 -*-
"""
Created on Mon Jul 24 12:36:30 2017
...
files = glob('./baldwin/*.htm')

# Extract text from Baldwin's pages:
def extract_html_2(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Baldwin's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    t = soup.find('h1',{"class":"main_page_title press-release-title"})
    title = t.get_text()
    title = re.sub('“|”', '', title)
    subfind = t.findNextSibling('h2')
    
    if subfind:
        subtitle = subfind.get_text()
    ...
    # Grab the text:
    contents = soup.find('div',{'id':'press'}).get_text()
    
    # Get rid of date and title:
    if subfind:
        datetitle = re.search(subtitle, contents)
    else: 
        datetitle = re.search(title, contents)
        
    if datetitle:
        contents = contents[datetitle.end():]
...
def clean_text_2(text, rm_prepost=True, rm_quotes=False):
    ...
    if rm_prepost:
        ...
        pre = re.search("^.*?\d{2}\.\d{2}\.\d{2}\s+(?=[A-Z])", text)
        
        # if we don't find anything, look for the dateline using all caps
        if pre == None:
            pre = re.search("^.*?WI\s+-\s+(?=[A-Z])", text)
