# -*- coding: utf-8 -*-
"""
Created on Tue Sep  5 13:46:59 2017
...
files = glob('./menendez/*.htm')

# Extract text from Menendez's pages:
def extract_html_61(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Menendez's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_61(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Menendez's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(NJ|N\.J\.|NEW JERSEY|New Jersey)\s+-\s+", "", text)
        ...
        # if that doesn't turn anything up, look for NJ dateline after caption:
        if pre == None:
            pre = re.search("^.*?[A-Za-z]*\s*[A-Za-z]*\,*\s*(NJ|N\.J\.|NEW JERSEY|New Jersey)\s+-\s+(?=[A-Z])", text)
