# -*- coding: utf-8 -*-
"""
Created on Fri Sep  8 14:52:50 2017
...
files = glob('./shaheen/*.htm')

# Extract text from Shaheen's pages:
def extract_html_85(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Shaheen's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # subset page further:
    soup = soup.find('div', {"class":"press-release"})
...
def clean_text_85(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Shaheen's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z]*\.*\s*){1,3}\,*\s*(NH|N\.H\.?|NEW HAMPSHIRE|New Hampshire)?\s+-\s+(?=[A-Z])", "", text)
