# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 14:03:03 2017
...
files = glob('./collins/*.htm')

# Extract text from Collins' pages:
def extract_html_21(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Collins' press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    post_date = soup.find('div',{"class":"submitted"})
    if post_date:
        post_date = post_date.get_text()
    else:
        return([docid, sen,title] + ['']*3)
    post_date = re.sub('^\\n.*\,\s+', '', post_date)
    post_date = re.sub('(?<=[0-9]{4}).*$', '', post_date)
    # Format the date:
...
def clean_text_21(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Collins' press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^.*?[A-Za-z]*\,*\s*(ME|MAINE|Maine)\s+-\s+(?=[A-Z])", "", text)
