# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 13:18:43 2017
...
files = glob('./portman/*.htm')

# Extract text from Portman's pages:
def extract_html_71(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Portman's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_71(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Portman's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(OH|OHIO|Ohio)?\s+-\s+(?=[A-Z])", "", text)
        ...
        if pre:
            ...
            # - smaller standard for Portman b/c he doesn't include (many/long)  
