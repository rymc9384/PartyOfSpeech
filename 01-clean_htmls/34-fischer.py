# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 15:08:16 2017
...
files = glob('./fischer/*.htm')

# Extract text from Fischer's pages:
def extract_html_34(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Fischer's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_34(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Fischer's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(Neb\.|NE|NEBRASKA|Nebraska)\s+-\s+", "", text)
