# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 12:27:39 2017
...
files = glob('./daines/*.htm')

# Extract text from Daines' pages:
def extract_html_28(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Daines' press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_28(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Daines' press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(Mont\.|MT|Montana)\s+-\s+", "", text)
        text = re.sub("^\(U\.S\. (Senate|Congress)\)\s+-\s+(?=[A-Z])", "", text)
        text = re.sub("^\(MONTANA\)\s+-\s+(?=[A-Z])", "", text)
