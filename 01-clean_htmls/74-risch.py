# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 16:19:39 2017
...
files = glob('./risch/*.htm')

# Extract text from Risch's pages:
def extract_html_74(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Risch's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    elif len(pars) == 1 and len(pars[0].get_text()) >= 10:
        contents = pars[0].get_text()
    elif len(pars) == 0:
...
def clean_text_74(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Risch's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^" + dc_dateline + "(\,?\s+[A-Za-z]{3,4}\.? \d{1,2})?\s+\-\s+(?=[A-Z])", "", text)
        text = re.sub("^(D\.C\.|D\.C|DC)\s+\-\s+(?=[A-Z])", "", text)
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(ID|IDAHO|Idaho)?\s+-\s+(?=[A-Z])", "", text)
        ...
        if pre:
            ...
            # - smaller standard for Risch b/c he doesn't include (many/long)  
