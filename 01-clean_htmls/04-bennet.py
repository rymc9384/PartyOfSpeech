# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 10:46:39 2017
...
files = glob('./bennet/*.htm')

# Extract text from Bennet's pages:
def extract_html_4(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Bennet's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Main content: (b/c of diff class names for titles)
    ...
    post_date = main.find('span', {"class":"info-date"}).get_text()
    ...
    # Grab the text:
    contents = main.find('div',{"class":"record-body clearfix"}).get_text()
...
def clean_text_4(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Bennet's press releases.
    #
    # - ARGUMENT(S): 
