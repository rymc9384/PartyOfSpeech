# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 12:05:25 2017
...
# Extract text from Cruz's pages:
def extract_html_27(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Cruz's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    soup = soup.find("div", {"id":"block5"})
    
    # Extract the date posted:
    post_date = soup.find('h3', {"id":"record-date"}).get_text()
    ...
    # Grab the text:
    main = soup.find('div', {"class":"record-body"})
...
def clean_text_27(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Cruz's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(TX|TEXAS|Texas)\s+-\s+", "", text)
        
        # look for the dateline using all caps
        pre = re.search("^.*?[A-Z]{4,}(?:\,\s+[A-Z]{2,})\s+-\s+(?=[A-Z])", text)
