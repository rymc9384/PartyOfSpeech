# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 09:45:23 2017
...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 09:31:42 2017
...
files = glob('./grassley/*.htm')

# Extract text from Grassley's pages:
def extract_html_39(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Grassley's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    soup = soup.find('div', {"class":"region-inner region-content-inner"})
    ...
    post_date = soup.find('span', {"class":"date-display-single"}).get_text()
    ...
    # Grab the text:
    main = soup.find('div', {"class":"field field-name-body field-type-text-with-summary field-label-hidden"})
    main = main.find('div', {"class":"field-item even"})
...
def clean_text_39(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Grassley's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(I\.A\.|IA|IOWA|Iowa)\s+-\s+", "", text)
        text = re.sub("^\(.*?\)\s+", "", text)
        
        # Dateline might be after a subtitle/header:
        pre = re.search("^.*?" + dc_dateline + "(\s+-\s+|\:\s+)(?=[A-Z])", text)
        
        # if we don't find anything, look for the dateline using dates
        if pre == None:
            pre = re.search("^([A-Z][a-z]+ )*(Statement|Remarks) (of|by).*?" + regex_months + \
                            "\s+\d{1,2}(?:(\,\s+\d{4}\s+)|" + \
                            "(\s+-\s+))(?=[A-Z])", text)
