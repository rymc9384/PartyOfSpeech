# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 14:14:03 2017
...
files = glob('./boxer/*.htm')

# Extract text from Boxer's pages:
def extract_html_10(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Boxer's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Extract the title:
    header = soup.find('div', {"class":"small-12 columns"})
    title = header.find('h1').get_text()
    title = re.sub('“|”', '', title)
    title = title.strip()
    
    # Find the body:
    main = soup.find('div', {"id":"block3"})
    if main == None:
        return([docid, sen, title] + ['']*3)

    # Extract the date posted:
    date = main.find('p').get_text()
    post_date = re.search('[A-Za-z]+ \d{1,2}\, \d{4}$', date)
...
def clean_text_10(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Boxer's press releases.
    #
    # - ARGUMENT(S): 
