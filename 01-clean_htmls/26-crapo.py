# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 11:17:55 2017
...
files = glob('./crapo/*.htm')

# Extract text from Crapo's pages:
def extract_html_26(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Crapo's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Extract the title:
    title = soup.find('span',{"title":"Release Title"})
    if title:
    ...
    post_date = soup.find('span', {"title":"Release Date"}).get_text()
    ...
    # Grab the text:
    main = soup.find('div', {"title":"Release Body"})
...
def clean_text_26(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Crapo's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z][a-z]*[\s]{0,1}){1,2}\s+-\s+(?=[A-Z])", "", text)
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(ID|IDAHO|Idaho)\s+-\s+", "", text)
