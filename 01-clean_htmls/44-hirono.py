# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 13:38:22 2017
...
files = glob('./hirono/*.htm')

# Extract text from Hirono's pages:
def extract_html_44(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Hirono's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_44(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Hirono's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z]*[\s]{0,1}){1,4}\,*\s*(HI|HAWAII|Hawaii)\s+-\s+(?=[A-Z])", \
                      "", text)        
