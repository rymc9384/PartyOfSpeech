# -*- coding: utf-8 -*-
"""
Created on Mon Sep 11 14:52:53 2017
...
files = glob('./thune/*.htm')

# Extract text from Thune's pages:
def extract_html_90(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Thune's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_90(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Thune's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(S\.?D\.?|SOUTH DAKOTA|South Dakota|[A-Z]{2})?\s+-\s+(?=(\"|[A-Z]))", "", text)
