# -*- coding: utf-8 -*-
"""
Created on Thu Sep 21 13:17:27 2017
...
files = glob('./warner/*.htm')

# Extract text from Warner's pages:
def extract_html_95(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Warner's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_95(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Warner's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        # First deal w/ troublemakers (newpaper articles/editorials):
        text = re.sub("E(DITORIAL|ditorial)\s(BOARD |Board )?", "", text)
        text = re.sub("^(B|b)y(\:)?( Senator|Sens\.)? \w+ ([A-Z]\.\s)?\w+(\;.*?\d{4}|\, \w+ \w+| and \w+ \w+| [A-Z]{2,10})?\s", "", text)
        text = re.sub("^(B|b)y.*?(and|\&) \w+\s(\w+\s)?", "", text)
        
        # Then just toss all the location lines
        text = re.sub("^" + dc_dateline + "\s+\-\s+(?=[A-Z])", "", text)
        text = re.sub("^(D\.C\.|D\.C|DC)\s+\-\s+(?=[A-Z])", "", text)
        text = re.sub("^([A-Z]*\.*\s*){1,3}\,*\s*(VA|VIRGINIA|Virginia|Va\.)?(\s\(AP\))?\s+-\s+(?=(\"|[A-Z]))", "", text)
        ...
        if pre == None:
            pre = re.search("^C(ONTACT|ontact)(.*?this number){1,3}\s([A-Z]+\s-\s)?(?=(\"|[A-Z]))", text)
