...
#'
#' \code{ExpAgendaVonmon} implements bayesian hierarchical topic model for texts from Grimmer (2010). Edited by Ryan McMahon, June 2017.
#'
...
ExpAgendaVonmon <- function(obj = NULL, term.doc = NULL, authors = NULL, n.cats = 10, kappa = 400, verbose = TRUE, tol_afr = 1e-5){
  ...
  if (!is.null(obj) & class(obj) != "ExpAgendaDTMatrix"){
    stop("obj must be of class ExpAgendaDTMatrix and created by PreProcess.")
  }
  ...
  for(j in 1:ncol(mus)){
    mus[,j]<- mus[,j]/ as.vector(sqrt(mus[,j]%*%mus[,j]))
  }
