# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 16:19:39 2017
...
files = glob('./cantwell/*.htm')

# Extract text from Cantwell's pages:
def extract_html_13(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Cantwell's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Remove title:
    title_search = r' '.join(title.split(' ')[-4:])
    
    contents = re.sub('^.*?' + title_search + '(?:\s+)', '', contents)
...
def clean_text_13(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Cantwell's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^.*?[A-Za-z]*\,*\s*(WA|Washington)\s+-\s+(?=[A-Z])", "", text)
