# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 14:41:12 2017
...
files = glob('./ernst/*.htm')

# Extract text from Ernst's pages:
def extract_html_32(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Ernst's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_32(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Ernst's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\,*\s*(IA|IOWA|Iowa)\s+-\s+(?=[A-Z])", "", text)        
