# -*- coding: utf-8 -*-
"""
Created on Fri Sep  8 11:26:31 2017
...
files = glob('./scott/*.htm')

# Extract text from Scott's pages:
def extract_html_83(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Scott's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_83(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Scott's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(SC|S\.C\.|SOUTH CAROLINA|South Carolina)?\s+-\s+(?=[A-Z])", "", text)
        ...
        # trouble makers:
        text = re.sub("^View.*?(the United States Capitol\:|Child Protection Act)\s(?=(Thank you|Mr\.))",
                      "", text)
        ...
        # if that is empty, look for a SC dateline after header:
        if pre == None:
            pre = re.search("^.*?([A-Za-z]*\.*\s*){1,3}\,*\s*(SC|S\.C\.|SOUTH CAROLINA|South Carolina)(\s+-|\:)\s+(?=([A-Z]|\"))", text)
        
        # lastly, if nothing is found, look for a date:
        if pre == None:
        ...
        if pre:
            ...
            # - smaller standard for Scott b/c he doesn't include (many/long)  
