# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 15:42:25 2017
...
files = glob('./kaine/*.htm')

# Extract text from Kaine's pages:
def extract_html_48(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Kaine's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_48(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Kaine's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z]*[a-z]*[\s]{0,1}){1,4}\,*\s*(VA|VIRGINIA|Va\.|Virginia)\s+-\s+(?=[A-Z])", \
                      "", text)        
        
        # Toss "Background: " for "Fact sheet" documents:
        text = re.sub("^Background\:\s?(?=\w+)", "", text)
        
        # Get rid of starting date:
        text = re.sub("^([A-Z]{1}[a-z]{2,}\,\s+)?" + regex_months + \
                        "\s+\d{1,2}(\,\s+\d{4})?" + \
                        "\s+-\s+(?=[A-Z])", "", text)
