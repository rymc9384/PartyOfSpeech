# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 14:09:02 2017
...
files = glob('./durbin/*.htm')

# Extract text from Durbin's pages:
def extract_html_30(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Durbin's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_30(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Durbin's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(Ill\.|IL|ILLINOIS|Illinois)\s+-\s+", "", text)
