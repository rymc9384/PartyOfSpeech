# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 12:45:54 2017
...
files = glob('./coats/*.htm')

# Extract text from Coat's pages:
def extract_html_19(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Coat's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Extract the title:
    title = soup.find('li',{"class":"current-section"})
    if title:
...
def clean_text_19(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Coat's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(Ind\.|IN|Indiana)\s+-\s+", "", text)
        text = re.sub("^INDIANAPOLIS\s+-\s+", "", text)
        
        # Remove the intro for letters:
        text = re.sub("^.*?(By|by) (?:(Senator|Sen\.)) Dan Coats\s+(?=[A-Z])", "", text)
