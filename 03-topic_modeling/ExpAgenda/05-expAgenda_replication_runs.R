...
## File: "~/03-topic_modeling/ExpAgenda/05-expAgenda_replication_runs.R"
##
## PURPOSE: This script fits the replication runs for the Expressed Agenda 
##          topic model (Grimmer 2010). These replication runs each use a new 
##          PRNG seed, which allows for new starting values. The "best run" 
##          will be used for analysis (i.e., the run w/ maximum lower-bound 
##          of evidence), following Grimmer (2010). 
##
## NOTES: 
##       10/13/17) This is adapted from the "01-estimating_ExpAgenda.R" script.
## 
##       10/13/17) Run on Penn State's HPC cluster. The software information 
...
##                 files.
## 
##       10/23/17) Add in the original model seed as the first seed so that we 
##                 evaluate all replications and original fit for the "best run".
##                  
##
## EDITS:
##        
...
option_list <- list(
  make_option(c("-n", "--nreps"), type="integer", default=100, 
              help="How many replications are being run [default=%default]", metavar="character"),
  make_option(c("-r", "--replication"), type="integer", default=1, 
              help="Replication run number (1 - 100) [default=%default]", metavar="character"),
...
## Set origin seed and generate `-n` other seeds:
set.seed(11769)
seeds <- sample(x = 1:100000, size = opt$nreps, replace = F)

## Add in origin seed (so we include the original model in replications)
seeds <- c( 11769, seeds )
...
## Fit model:
set.seed(seeds[opt$replication])
cat(paste0("\n\n\tFitting replication ", opt$replication, " with ", opt$topics, " topics, a kappa of ", opt$kappa, ", and delta of ", opt$delta, "...\n"))
