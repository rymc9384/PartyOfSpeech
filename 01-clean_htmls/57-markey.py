# -*- coding: utf-8 -*-
"""
Created on Fri Sep  1 15:15:55 2017
...
files = glob('./markey/*.htm')

# Extract text from Markey's pages:
def extract_html_57(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Markey's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_57(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Markey's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z]*[\s]{0,1}){1,4}\,*\s*(MA|MASS|MASSACHUSETTS|Mass\.|Massachusetts)\s+-\s+(?=[A-Z])", \
                      "", text)        
        text = re.sub("^(Boston|BOSTON)(\, MA)?(\s\(.*?\d{4}\))?\s+(-\s+)?(?=[A-Z])", "", text)
        
        # search for DC dateline after header:
        dc_dateline2 = dc_dateline[:-11] + dc_dateline[-1] # removes all caps search
        pre = re.search("^.*?" + dc_dateline2 + "(\]|\))?(\s+\(.*?\))?\s+(-\s+)?(?=[A-Z])", text)
        
        # if that doesn't find anything, look for dates indicating start:
        if pre == None:
            pre = re.search("^.*?([A-Z]{5,}|\, MA) \(" + regex_months + " \d{1,2}\, \d{4}\)\s+-\s+(?=[A-Z])", \
                            text)
    
        # if either turns something up, do some diagnostics:
        if pre:
            pre_span = pre.span()
