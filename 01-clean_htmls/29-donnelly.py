# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 12:41:09 2017
...
files = glob('./donnelly/*.htm')

# Extract text from Donnelly's pages:
def extract_html_29(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Donnelly's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_29(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Donnelly's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z][a-z]*[\s]{0,1}){1,4}\,*\s*(Ind\.|IN|INDIANA|Indiana)\s+-\s+(?=[A-Z])", \
                      "", text)
        text = re.sub("^INDIANAPOLIS\s+-\s+(?=[A-Z])", "", text)
        text = re.sub("^([A-Z][a-z]*[\s]{0,1}){1,3}(?:\,\s[A-Z][a-z]*)\s+-\s+(?=[A-Z])", "", text)
