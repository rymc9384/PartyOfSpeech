# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 11:10:50 2017
...
files = glob('./capito/*.htm')

# Extract text from Capito's pages:
def extract_html_14(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Capito's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_14(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Capito's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^.*?[A-Za-z]*\,*\s*(W\.Va\.|WV)\s+-\s+(?=[A-Z])", "", text)
