# -*- coding: utf-8 -*-
"""
Created on Thu Sep  7 11:16:14 2017
...
files = glob('./rubio/*.htm')

# Extract text from Rubio's pages:
def extract_html_78(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Rubio's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    post_date = soup.find('h4', {"class":"byline-date"}).get_text()
    post_date = post_date.strip()
    
    if len(post_date) > 12:
        post_date = re.search("[A-Z][a-z]+\s+\d{2}\s+\d{4}", post_date)
...
def clean_text_78(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Rubio's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        # First, get rid of intro punctuation:
        text = re.sub('^\.+\s(?=([A-Z]|\"))', '', text)
        
        # Then just toss all the location lines
        text = re.sub("^" + dc_dateline + "(\,?\s+[A-Za-z]* \d{1,2}[sthrd]{0,2})?\s+(\-\s+)?(?=([A-Z]|\"))", "", text)
        text = re.sub("^(D\.C\.|D\.C|DC)\s+\-\s+(?=[A-Z])", "", text)
        text = re.sub("^([A-Za-z]*\s*){1,3}\,*\s*(FL|FLORIDA|Florida)?(\s+\(.*?\d{4}\))?\s+-\s+(?=([A-Z]|\"))", "", text)
        ...
        # if that comes up empty, look for date and newspaper title:
        if pre == None:
            pre = re.search("^.*?\,\s" + regex_months + "\s\d{1,2}\,\s\d{4}\s([A-Z][a-z]*[\s]{0,1}){2,4}(?=[A-Z][a-z]*\s[a-z])", text)
        ...
        if pre:
            ...
            # - smaller standard for Johnson b/c he doesn't include (many/long)  
