# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 15:26:57 2017
...
files = glob('./isakson/*.htm')

# Extract text from Isakson's pages:
def extract_html_47(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Isakson's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_47(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Isakson's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(GA|GEORGIA)\s+-\s+", "", text)
        ...
        pre = re.search("^.*([A-Z]*[\s]{0,1}){1,4}\,*\s*(GA|GEORGIA)\s+-\s+(?=[A-Z])", \
                        text)
        
        # look for just Atlanta, no state info:
        if pre == None:
            pre = re.search("^.*?ATLANTA\s+-\s+(?=[A-Z])", text)
        
        # if those don't turn something up, check for DC dateline:
        if pre == None:
