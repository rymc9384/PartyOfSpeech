# -*- coding: utf-8 -*-
"""
Created on Tue Sep  5 12:46:30 2017
...
files = glob('./mccain/*.htm')

# Extract text from McCain's pages:
def extract_html_58(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator McCain's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_58(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from McCain's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(AZ|ARIZONA)\s+-\s+", "", text)
        
        text = re.sub("^" + regex_months + \
                        "\s+\d{1,2}(\,\s+\d{4})?" + \
                        "\s+-\s+(?=[A-Z])", "", text)
        
        # look for atypical dateline:
        ...
        if pre:
            ...
            # - smaller standard for McCain b/c he doesn't include headers in 
