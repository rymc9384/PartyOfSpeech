...
files = glob('./warren/*.htm')

# Extract text from Warren's pages:
def extract_html_96(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Warren's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # 
    # - NOTES: './warren/280warren.htm' is an empty file.
    # 
    ###########################################################################
    ...
    title = soup.find('h1',{"class":"page-title fancy_font"}).get_text()
    title = re.sub('“|”', '', title)
    
    post_date = soup.find('h3',{"class":"inner-lyt-dates"}).get_text()
    ...
    # Grab the text:
    main = soup.find('div',{'class':'bd record-bd'})
...
def clean_text_96(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Warren's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(MA|MASSACHUSETTS|Massachusetts|Mass\.)?\s+-\s+(?=[A-Z])", "", text)
        ...
        if pre:
            ...
            # shorter standard for Warren b/c those headers are very rare & short
            if pre_frac < 0.10:
