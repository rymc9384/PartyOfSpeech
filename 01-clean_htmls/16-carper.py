# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 11:47:00 2017
...
files = glob('./carper/*.htm')

# Extract text from Carper's pages:
def extract_html_16(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Carper's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_16(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Carper's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Z]*\,*\s*(DE|Del\.)\s+-\s+", "", text)
        text = re.sub("^WILMINGTON\s+-\s+", "", text)
