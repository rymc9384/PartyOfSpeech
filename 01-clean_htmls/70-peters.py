# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 12:58:12 2017
...
files = glob('./peters/*.htm')

# Extract text from Peters' pages:
def extract_html_70(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Peters' press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    #
    # - NOTES:
    #           09/06/2017: './peters\\192peters.htm' is empty.
    ###########################################################################
...
def clean_text_70(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Peters' press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(MI|MICHIGAN|Mich\.|Michigan)?\s+-\s+(?=[A-Z])", "", text)
