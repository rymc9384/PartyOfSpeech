# -*- coding: utf-8 -*-
"""
Created on Fri Sep  1 15:05:45 2017
...
files = glob('./manchin/*.htm')

# Extract text from Manchin's pages:
def extract_html_56(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Manchin's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_56(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Manchin's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\s*){1,3}\,*\s*(WV|W\.Va\.|W\.V\.)\s+-\s+(?=[A-Z])", "", text)
        
        # look for dateline after header:
        pre = re.search("^.*?[A-Za-z]*\s*[A-Za-z]*\,*\s*(WV|W\.Va\.|W\.V\.)\s+-\s+(?=[A-Z])", text)
        
        # if that doesn't turn something up, check for DC dateline:
        if pre == None:
            pre = re.search("^.*?(?<!=\.)\s\(?" + dc_dateline + "\)?\s+\-\s+(?=[A-Z])", text)
