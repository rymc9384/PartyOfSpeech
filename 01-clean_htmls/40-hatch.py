# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 10:16:58 2017
...
files = glob('./hatch/*.htm')

# Extract text from Hatch's pages:
def extract_html_40(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Hatch's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_40(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Hatch's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\s*){1,3}\,*\s*(UT|UTAH|Utah)\s+-\s+", "", text)
        text = re.sub("^SALT LAKE CITY\s+-\s+(?=[A-Z])", "", text)
        text = re.sub("^For Immediate Release\:.*?" + dc_dateline + "\s+\-\s+(?=[A-Z])", "", text)
        
        # look for header with all caps:
        pre = re.search("^([A-Z]|\W)*(?=[A-Z][a-z])", text)
