# -*- coding: utf-8 -*-
"""
Created on Mon Sep 11 15:26:44 2017
...
########################
### 92) Senator Tom Udall:
    
files = glob('./tomudall/*.htm')

# Extract text from Tom Udall's pages:
def extract_html_92(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Tom Udall's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_92(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Tom Udall's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(N\.?M\.?|NEW MEXICO|New Mexico)?\s+-\s+(?=[A-Z])", "", text)
        ...
        if pre:
            ...
            # shorter standard for Udall b/c those headers are very rare & short
            if pre_frac < 0.10:
