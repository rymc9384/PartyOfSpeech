# -*- coding: utf-8 -*-
"""
Created on Fri Sep  1 09:42:15 2017
...
files = glob('./lankford/*.htm')

# Extract text from Lankford's pages:
def extract_html_52(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Lankford's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_52(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Lankford's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z]*[\s]{0,1}){1,4}\,*\s*(OKLA\.|OK|OKLAHOMA|Okla\.|Oklahoma)\s+-\s+(?=[A-Z])", "", text)
        ...
        pre = re.search("^.*?[A-Za-z]*\s*[A-Za-z]*\,*\s*(OKLA\.|OK|OKLAHOMA|Okla\.|Oklahoma)\s+-\s+(?=[A-Z])", text)
