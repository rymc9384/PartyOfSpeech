# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 15:56:55 2017
...
# Extract text from Reid's pages:
def extract_html_73(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Reid's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Extract the title:
    title = soup.find('h3',{"class":"entry-title single-title"})
    if title:
    ...
    post_date = soup.find('div',{"class":"byline vcard clearfix"}).get_text()
    ...
    # Grab the text:
    main = soup.find('section',{"class":"entry-content clearfix"})
...
def clean_text_73(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Reid's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(NV|N\.V\.|NEVADA|Nevada)?\s+-\s+(?=(\"|[A-Z]))", "", text)
        ...
        if pre:
            ...
            # - smaller standard for Reid b/c he doesn't include (many/long)  
