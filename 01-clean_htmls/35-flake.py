# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 15:13:42 2017
...
files = glob('./flake/*.htm')

# Extract text from Flake's pages:
def extract_html_35(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Flake's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_35(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Flake's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(Ariz\.|AZ|ARIZONA|Arizona)\s+-\s+", "", text)
