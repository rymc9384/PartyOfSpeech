# -*- coding: utf-8 -*-
"""
Created on Thu Sep  7 10:43:22 2017
...
########################
### 76) Senator Ron Johnson:
    
files = glob('./ronjohnson/*.htm')

# Extract text from Ron Johnson's pages:
def extract_html_76(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Ron Johnson's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_76(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Ron Johnson's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\s*){1,3}\,*\s*(WI|WISCONSIN|Wis\.|Wisconsin)?(\s+\(.*?\d{4}\))?\s+-\s+(?=([A-Z]|\"))", "", text)
        ...
        # get rid of byline:
        text = re.sub("^By ([A-Z][a-z]{2,}\s*){2} http.*?\s+(?=[A-Z])", "", text)
