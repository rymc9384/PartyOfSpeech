# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 10:18:54 2017
...
files = glob('./murphy/*.htm')

# Extract text from Murphy's pages:
def extract_html_66(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Murphy's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Grab the text:
    main1 = soup.find('div',{"class":"offset2 row-fluid span12"})
...
def clean_text_66(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Murphy's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^\(?([A-Za-z]*[\s]{0,1}){1,4}\,*\s*(CT|CONN\.|CONNECTICUT|Conn\.|Connecticut)?\)?\s+-\s+(?=[A-Z])", \
                      "", text)        
        
        # Remove letter caption:
        text = re.sub("^Full text of the letter\:\s+(?=[A-Z])", "", text)
        ...
        # if that doesn't turn anything up, look for CT dateline after header:
        if pre == None:
            pre = re.search("^.*?([A-Za-z]*[\s]{0,1}){1,4}\,*\s*(CT|CONN\.|CONNECTICUT|Conn\.|Connecticut)\s+-\s+(?=[A-Z])", text)
