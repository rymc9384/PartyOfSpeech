# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 12:30:51 2017
...
files = glob('./cassidy/*.htm')

# Extract text from Cassidy's pages:
def extract_html_18(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Cassidy's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Grab the text:
    contents = soup.find('td', {"class":"td1"})
    if not contents:
...
def clean_text_18(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Cassidy's press releases.
    #
    # - ARGUMENT(S): 
