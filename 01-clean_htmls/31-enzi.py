# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 14:29:02 2017
...
# Extract text from Enzi's pages:
def extract_html_31(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Enzi's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    post_date = soup.find('h2',{"class":"date"}).get_text()
    post_date = re.sub(r'\n', '', post_date)
...
def clean_text_31(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Enzi's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\,*\s*(Wyo\.|WY|WYOMING|Wyoming)\s+-\s+(?=[A-Z])", "", text)        
        text = re.sub("^SENATOR ENZI\:\s+", "", text)
