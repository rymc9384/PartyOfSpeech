# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 11:07:38 2017
...
files = glob('./murray/*.htm')

# Extract text from Murray's pages:
def extract_html_67(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Murray's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    if main:
        ...
        return([docid, sen, post_date, form_date, title, ""])
...
def clean_text_67(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Murray's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^\(?([A-Za-z]*[\s]{0,1}){1,3}\,*\s*(WA|WASH\.|WASHINGTON|Wash\.|Washington)?\)?\s+-\s+(?=[A-Z])", \
                      "", text)
        
        # toss header dates if they're there
        text = re.sub("^" + regex_months + \
                        "\s+\d{1,2}(\,\s+\d{4})?" + \
                        "\s+-\s+(?=[A-Z])", "", text)
        
        # get rid of "watch this video" stuff at start
        text = re.sub("^W(ATCH|atch).*?\s+-\s+(?=[A-Z])", "", text)
        
        # get rid of "key exceprts" stuff at start
        text = re.sub("^Key excerpts.*?\:\s+(?=([A-Z]|\"))", "", text)
        ...
        # if that doesn't turn anything up, look for WA dateline after header:
        if pre == None:
            pre = re.search("^.*?([A-Z]{2,}[\s]{0,1}){1,3}\,*\s*(WA|WASH\.|WASHINGTON|Wash\.|Washington)?\)?\s+-\s+(?=[A-Z])(?!=WA\))", text)
        ...
        if pre:
            ...
            # - smaller standard for Murray b/c she doesn't include (many/long)  
