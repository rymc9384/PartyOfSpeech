# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 13:56:21 2017
...
files = glob('./hoeven/*.htm')

# Extract text from Hoeven's pages:
def extract_html_45(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Hoeven's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_45(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Hoeven's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(N\.D\.|ND|NORTH DAKOTA|North Dakota)\s+-\s+", "", text)
                
        # Trouble maker:
        text = re.sub("^GROTON\, CONN\.\s+-\s+(?=[A-Z])", "", text)
        ...
        pre = re.search("^.*?[A-Za-z]*\s*[A-Za-z]*\,*\s*(N\.D\.|ND|NORTH DAKOTA|North Dakota)\s+-\s+(?=[A-Z])", text)
        
        # if that doesn't turn something up, check for DC dateline:
        if pre == None:
            pre = re.search("^.*?(D\.C\.|D\.C|DC)\s+\-\s+(?=[A-Z])", text)
