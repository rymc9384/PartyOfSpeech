...
########################
### 38) Senator Gillibrand:
    
files = glob('./gillibrand/*.htm')

# Extract text from Gillibrand's pages:
def extract_html_38(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Gillibrand's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # Extract the title:
    title = soup.find('div', {"class":"headline"})
    if title:
    ...
    # Date is in same section as text:
    pars = soup.find_all('p')

    # Extract the date posted:
    post_date = pars[0].get_text()
    pars.pop(0)
...
def clean_text_38(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Gillibrand's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(N\.Y\.|NY|NEW YORK|New York)\s+-\s+", "", text)
