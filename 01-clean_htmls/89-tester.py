# -*- coding: utf-8 -*-
"""
Created on Mon Sep 11 14:37:19 2017
...
files = glob('./tester/*.htm')

# Extract text from Tester's pages:
def extract_html_89(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Tester's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    title = soup.find('h1',{"class":"fancy_font"})
    if title:
...
def clean_text_89(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Tester's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z]*\.*\s*){1,3}\,*\s*(MT|MONTANA|Mont\.|Montana)?\s+-\s+(?=[A-Z])", "", text)
