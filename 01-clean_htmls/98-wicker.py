# -*- coding: utf-8 -*-
"""
Created on Thu Sep 21 15:21:52 2017
...
files = glob('./wicker/*.htm')

# Extract text from Wicker's pages:
def extract_html_98(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Wicker's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_98(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Wicker's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(MISS\.|MISSISSIPPI|MS\.?|Mississippi|[A-Z]{2,10})?\s+-\s+(?=(\"|[A-Z]))", "", text)
