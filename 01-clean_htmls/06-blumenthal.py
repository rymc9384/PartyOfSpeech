# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 12:05:33 2017
...
########################
### 6) Senator Blumenthal:
    
files = glob('./blumenthal/*.htm')

# Extract text from Blumenthal's pages:
def extract_html_6(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Blumenthal's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    # 
    # - NOTES: 
    #       08/28/17) './blumenthal/1142blumenthal.htm' appears to have been  
    ...
    post_date = soup.find('time').get_text()
    ...
    # Grab the text:
    main = soup.find('article',{"id":"main"})
...
def clean_text_6(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Blumenthal's press releases.
    #
    # - ARGUMENT(S): 
