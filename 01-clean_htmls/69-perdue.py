# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 12:35:34 2017
...
files = glob('./perdue/*.htm')

# Extract text from Perdue's pages:
def extract_html_69(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Perdue's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_69(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Perdue's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Z]*\s*){1,3}\,*\s*(GA|GEORGIA|Ga\.|Georgia)?\s+-\s+(?=[A-Z])", "", text)

        text = re.sub("^" + regex_months + \
                        "\s+\d{1,2}(\,\s+\d{4})?" + \
                        "\s+-\s+(?=[A-Z])", "", text)
        
        # look for dateline after small header:
        pre = re.search("^.*? \d{1,2} \([A-Z]+\)\s+\-\s+(?=[A-Z])", text)
        ...
        if pre:
            ...
            # - smaller standard for Perdue b/c he doesn't include (many/long)  
