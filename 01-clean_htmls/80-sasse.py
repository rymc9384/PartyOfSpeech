# -*- coding: utf-8 -*-
"""
Created on Thu Sep  7 13:54:16 2017
...
files = glob('./sasse/*.htm')

# Extract text from Sasse's pages:
def extract_html_80(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Sasse's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_80(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Sasse's press releases.
    #
    # - ARGUMENT(S): 
    ...
    # A few bad apples:
    text = re.sub('^' + r'// <!' + '.*?Remarks as prepared for delivery\.\s+', 
                                '', text)
    text = re.sub('^CANCELED.*?\.\s+(?=[A-Z])', '', text)
    text = re.sub('^by U.S. Senator Ben Sasse\s(?=[A-Z])', '', text)
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(NE|NEBRASKA|Nebraska)?\s+-\s+(?=(\"|[A-Z]))", "", text)
        ...
        if pre:
            ...
            # - smaller standard for Sasse b/c he doesn't include (many/long)  
            # headers in the `contents` section:
            if pre_frac < 0.10:
                text = text[pre_span[1]:]
        
        # look for end by cutting out after twitter pic
        post = re.search("(https\:|http\:)?(pic\.)?twitter\.com.*?$", text)
        # else, look for the text at the end using the octothorpe/pound sign
        if post == None:
