# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 15:41:43 2017
...
files = glob('./corker/*.htm')

# Extract text from Corker's pages:
def extract_html_23(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Corker's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_23(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Corker's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^.*?[A-Za-z]*\,*\s*(Tenn\.|TN|Tennessee)\s+-\s+(?=[A-Z])", "", text)
