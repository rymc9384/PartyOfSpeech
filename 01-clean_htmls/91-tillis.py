# -*- coding: utf-8 -*-
"""
Created on Mon Sep 11 15:09:39 2017
...
files = glob('./tillis/*.htm')

# Extract text from Tillis' pages:
def extract_html_91(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Tillis' press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_91(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Tillis' press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(N\.?C\.?|NORTH CAROLINA|North Carolina)?\s+-\s+(?=(\"|[A-Z]))", "", text)

        # All cap headers:
        text = re.sub("^([A-Z]*[\s]{0,1})+\:\s(?=[A-Z])", "", text)
        ...
        if pre:
            ...
            # shorter standard for Tillis b/c those headers are very rare & short
            if pre_frac < 0.10:
