...
class POSModel(object):
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("chars"):
            if self.config.chars:
                ...
                # bi lstm on chars
                lstm_cell1 = tf.contrib.rnn.LSTMCell(self.config.char_hidden_size, 
                                                    state_is_tuple=True)
                lstm_cell2 = tf.contrib.rnn.LSTMCell(self.config.char_hidden_size, 
                                                    state_is_tuple=True)
                _, ((_, output_fw), (_, output_bw)) = tf.nn.bidirectional_dynamic_rnn(lstm_cell1, 
                    lstm_cell2, char_embeddings, sequence_length=word_lengths, 
                    dtype=tf.float32)
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("bi-lstm"):
            lstm_cell1 = tf.contrib.rnn.LSTMCell(self.config.hidden_size)
            lstm_cell2 = tf.contrib.rnn.LSTMCell(self.config.hidden_size)
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(lstm_cell1, 
                lstm_cell2, self.word_embeddings, sequence_length=self.sequence_lengths, 
                dtype=tf.float32)
            output = tf.concat([output_fw, output_bw], axis=-1)
