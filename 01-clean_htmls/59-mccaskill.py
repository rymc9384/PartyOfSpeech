# -*- coding: utf-8 -*-
"""
Created on Tue Sep  5 13:11:15 2017
...
########################
### 59) Senator McCaskill:
    
files = glob('./mccaskill/*.htm')

# Extract text from McCaskill's pages:
def extract_html_59(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator McCaskill's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_59(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from McCaskill's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(MO|MISSOURI)\s+-\s+", "", text)
        text = re.sub("^([A-Z]*[\s]{0,1}){1,4}\s+-\s+(?=(\"|[A-Z]))", "", text)
        text = re.sub("^ST\. LOUIS\s+-\s+(?=[A-Z])", "", text)
        
        # remove op-ed header:
        text = re.sub("^FOR PUBLICATION .*?words\)\s+(?=[A-Z])", "", text)
