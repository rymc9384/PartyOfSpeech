# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 12:07:31 2017
...
files = glob('./casey/*.htm')

# Extract text from Casey's pages:
def extract_html_17(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Casey's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    #
    # - NOTES: 
    #       08/29/17) some pages are just a picture and link to story;
    #                 skip based on finding title.
    ###########################################################################
    ...
    # Grab the text:
    main = soup.find('div',{"class":"span8"})
...
def clean_text_17(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Casey's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\,*\s*(PA|Pennsylvania)\s+-\s+", "", text)
