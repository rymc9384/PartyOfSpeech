# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 13:46:13 2017
...
files = glob('./cochran/*.htm')

# Extract text from Cochran's pages:
def extract_html_20(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Cochran's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    #
    # Are these pages supposed to be readable?! Current pgs are much better.
    ###########################################################################
    ...
    post_date_raw = soup.find('h4', {"class":"date"}).get_text()
...
def clean_text_20(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Cochran's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(Miss\.|MS|Mississippi)\s+-\s+", "", text)
