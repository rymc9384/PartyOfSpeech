# -*- coding: utf-8 -*-
"""
Created on Fri Sep  1 09:12:15 2017
...
########################
### 51) Senator Klobuchar:
    
files = glob('./klobuchar/*.htm')

# Extract text from Klobuchar's pages:
def extract_html_51(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Klobuchar's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    post_date= soup.find('h4', {"class":"datetime"}).get_text()
...
def clean_text_51(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Klobuchar's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(Minn\.|MN|MINNESOTA|Minnesota)\s+-\s+(?=[A-Z])", "", text)
        
        text = re.sub("^" + regex_months + \
                        "\s+\d{1,2}(\,\s+\d{4})?" + \
                        "\s+-\s+(?=[A-Z])", "", text)
        
        # look for Minnesota dateline after header:
        pre = re.search("^.*([A-Z]*[\s]{0,1}){1,4}\,*\s*(Minn\.|MN|MINNESOTA|Minnesota)\s+-\s+(?=[A-Z])", text)
        
        # if that doesn't find anything, check for DC dateline after a header:
        if pre == None:
