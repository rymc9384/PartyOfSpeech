# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 14:54:19 2017
...
########################
### 33) Senator Feinstein:
    
files = glob('./feinstein/*.htm')

# Extract text from Feinstein's pages:
def extract_html_33(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Feinstein's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_33(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Feinstein's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^[A-Za-z]*\s*[A-Za-z]*\,*\s*(Cal\.|CA|CALIFORNIA|California)\s+-\s+", "", text)
