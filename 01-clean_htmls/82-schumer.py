# -*- coding: utf-8 -*-
"""
Created on Fri Sep  8 11:10:59 2017
...
files = glob('./schumer/*.htm')

# Extract text from Schumer's pages:
def extract_html_82(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Schumer's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_82(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Schumer's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(NY|N\.Y\.|NEW YORK|New York)?\s+-\s+(?=[A-Z])", "", text)
        
        # weird one:
        text = re.sub('^ON - HUDSON\, NY - (?=[A-Z])', '',text)        
