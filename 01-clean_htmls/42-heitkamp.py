# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 13:08:18 2017
...
files = glob('./heitkamp/*.htm')

# Extract text from Heitkamp's pages:
def extract_html_42(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Heitkamp's press release HTMLs.
    #
    # - ARGUMENT(S): 
...
def clean_text_42(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Heitkamp's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        pre = re.search("^.*?[A-Za-z]*\s*[A-Za-z]*\,*\s*(N\.D\.|ND|NORTH DAKOTA|North Dakota)\s+-\s+", text)
