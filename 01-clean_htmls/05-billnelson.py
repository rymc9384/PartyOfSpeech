# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 11:05:44 2017
...
files = glob('./billnelson/*.htm')

# Extract text from Bill Nelson's pages:
def extract_html_5(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Bill Nelson's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    post_date = soup.find('div', {"class":"pane-content"}).get_text().strip()
    ...
    # Grab the text:
    contents_raw = soup.find_all('div',{"class":"field-item even"})
    if len(contents_raw) > 1:
        contents_raw = contents_raw[1]
    else:
        contents_raw = contents_raw[0]
        
    contents = contents_raw.get_text()
    
    # Remove extra subtitles:
    strong = contents_raw.find('strong')
    if strong:
        contents = re.sub('^'+strong.get_text(), '', contents)
        emph = contents_raw.find('em')
        if emph:
            emph = emph.get_text()
            contents = re.sub(emph + '.*?\n', '', contents)
...
def clean_text_5(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Bill Nelson's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        if pre:
            ...
            # if 'pre' comprises less than 25% of full text AND is shorter than
            # 450 characters, remove it
            if pre_frac < 0.25 and pre_len < 450:
