# -*- coding: utf-8 -*-
"""
Created on Fri Sep  8 16:02:13 2017
...
files = glob('./sullivan/*.htm')

# Extract text from Sullivan's pages:
def extract_html_88(file):
    
    ###########################################################################   
    # - PURPOSE: Extract text from Senator Sullivan's press release HTMLs.
    #
    # - ARGUMENT(S): 
    ...
    #
    # - NOTES:
    #       09/08/17: './sullivan\\105sullivan.htm' is empty.
    ###########################################################################
    ...
    soup_test = soup.find('div', {"id":"press"})
    ...
    # Otherwise see if it's a newspaper article:
    else:
        title = soup.find('div', {"class":"wl-title"})
        if title:
            title = title.get_text()
            
            post_date = soup.find('span', {"class":"wl-startdate"}).get_text()
            ...
            contents = soup.find('div', {"class":"p402_premium"}).get_text()
        else:
            return([docid, sen] + ['']*4)
    ...
    # get rid of date and title:
    if soup_test:
        contents = re.sub('^.*?' + title[-8:], '', contents)
...
def clean_text_88(text, rm_prepost=True, rm_quotes=False):
    
    ###########################################################################   
    # - PURPOSE: Clean text extracted from Sullivan's press releases.
    #
    # - ARGUMENT(S): 
    ...
    if rm_prepost:
        ...
        # First, deal with bad apples:
        text = re.sub("^(Fairbanks Daily )?News - Miner (opinion|editorial)\:\s", "", text)
        text = re.sub("^Mr\. SULLIVAN\.\s", "", text)
        text = re.sub("^Remarks prepared for delivery\:\s", "", text)
        
        # Then toss all the location lines
        text = re.sub("^" + dc_dateline + "\,?\s+(\-\s+)?(?=[A-Z])", "", text)
        text = re.sub("^(D\.C\.|D\.C|DC)\s+\-\s+(?=[A-Z])", "", text)
        text = re.sub("^([A-Za-z]*\.*\s*){1,3}\,*\s*(AK|Alaska|Alaska)?\s+-\s+(?=(\"|[A-Z]))", "", text)
        ...
        # if that is empty, look for a AK dateline after header:
        if pre == None:
            pre = re.search("^.*?([A-Za-z]*\.*\s*){1,3}\,*\s*(AK|Alaska|Alaska)\)?\s+-\s+(?=([A-Z]|\"))", text)
